...
from Hackfreaks.modules.log_channel import gloggable, loggable
...
def ban(update: Update, context: CallbackContext) -> str:
    ...
    if is_user_ban_protected(chat, user_id, member) and user not in DEV_USERS:
        if user_id == OWNER_ID:
            ...
            message.reply_text(
                "Fighting this Elite here will put civilian lives at risk.")
            return log_message
        elif user_id in DEMONS:
            message.reply_text(
                "Bring an order from the EMPEROR to fight a Knight."
            )
            return log_message
        elif user_id in TIGERS:
            message.reply_text(
                "Bring an order from the EMPEROR to fight a Horsemen."
            )
            return log_message
...
def hit(update: Update, context: CallbackContext) -> str:
    ...
    if is_user_ban_protected(chat, user_id):
        message.reply_text("I really wish I could hit this user....")
        return log_message
    ...
    if res:
        ...
        message.reply_text("Well damn, I can't hit that user.")
...
def hitme(update: Update, context: CallbackContext):
    ...
    if res:
        update.effective_message.reply_text("*hites you out of the group*")
    else:
